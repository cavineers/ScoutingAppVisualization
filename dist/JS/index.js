...
function baseLine() {
    ...
    MongoClient.connect(mongoUrl, function(err, db) {
        ...
        setTimeout(function() {
            ...
            document.getElementById('rankingData').innerHTML = `<span style="font-size: 24px;">Average Pick Ups: <span style="color: yellow;">${pikCellsParse}</span><br>Average Delivers to Level 1: <span style="color: yellow;">${delCells1Parse}</span><br>Average Delivers to Level 2: <span style="color: yellow;">${delCells2Parse}</span><br>Average Delivers to Level 3: <span style="color: yellow;">${delCells3Parse}</span><br>Average Stage 2 Spin (3-5): <span style="color: yellow;">${stage2}%</span><br>Average Lands on Correct Color: <span style="color: yellow">${stage3}%</span><br>Average Climb: <span style="color: yellow;">${climb}%</span><br>Average Pins: <span style="color: yellow;">${pinsParse}</span><br>Average Push: <span style="color: rgb(3, 223, 252);">${pushParse}</span><br>Average Disables: <span style="color: rgb(3, 223, 252);">${disabledParse}</span></span>`;
            document.getElementById('loading2').style.display = "none";
        }, 900);
    })
}